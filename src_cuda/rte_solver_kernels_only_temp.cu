...
#include "Array.h"
    ...
    template<typename TF, BOOL_TYPE top>__device__
                          ...
                          TF* __restrict__ albedo, TF* __restrict__ src, TF* __restrict__ denom)
    {
        if (top)
        {
           ...
           #pragma unroll loop_unroll_factor_n
    ...
    template<typename TF, BOOL_TYPE top>__global__
                                 ...
                                 TF* __restrict__ albedo, TF* __restrict__ src, TF* __restrict__ denom)
    {
        ...
        const int igpt = blockIdx.y*block_size_y + threadIdx.y;

        if ( (icol < ncol) && (igpt < ngpt) )
        {
            sw_source_kernel<TF, top>(icol, igpt, ncol, nlay, top_at_1, r_dir, t_dir,
                             t_noscat, sfc_alb_dir, source_up, source_dn, source_sfc, flux_dir);

            sw_adding_kernel<TF, top>(icol, igpt, ncol, nlay, top_at_1, sfc_alb_dif,
