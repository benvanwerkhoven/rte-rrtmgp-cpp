...
namespace rrtmgp_kernel_launcher
{
    void increment_1scalar_by_1scalar(
            ...
            Array<Real,3>& tau_inout, const Array<Real,3>& tau_in)

    {
        rrtmgp_kernels::increment_1scalar_by_1scalar(
                &ncol, &nlay, &ngpt,
                tau_inout.ptr(), const_cast<Real*>(tau_in.ptr()));
    }

    void increment_2stream_by_2stream(
            int ncol, int nlay, int ngpt,
            Array<Real,3>& tau_inout, Array<Real,3>& ssa_inout, Array<Real,3>& g_inout,
            const Array<Real,3>& tau_in, const Array<Real,3>& ssa_in, const Array<Real,3>& g_in)
    {
        rrtmgp_kernels::increment_2stream_by_2stream(
                &ncol, &nlay, &ngpt,
                tau_inout.ptr(), ssa_inout.ptr(), g_inout.ptr(),
                const_cast<Real*>(tau_in.ptr()), const_cast<Real*>(ssa_in.ptr()), const_cast<Real*>(g_in.ptr()));
    }
...
void Optical_props_2str::delta_scale(const Array<Real,3>& forward_frac)
{
