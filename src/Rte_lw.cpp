...
{
    // CvH: This needs a more elegant solution, move it out of these files.
    // apply_BC is used in LW and SW solver.
    void apply_BC(
    ...
    void lw_solver_noscat_GaussQuad(
            ...
            const Array<Real,2>& gauss_Ds_subset,
            const Array<Real,2>& gauss_wts_subset,
            const Array<Real,3>& tau,
            const Array<Real,3>& lay_source,
            const Array<Real,3>& lev_source_inc, const Array<Real,3>& lev_source_dec,
            const Array<Real,2>& sfc_emis_gpt, const Array<Real,2>& sfc_source,
            Array<Real,3>& gpt_flux_up, Array<Real,3>& gpt_flux_dn,
            Array<Real,2>& sfc_source_jac, Array<Real,3>& gpt_flux_up_jac)
{
    rrtmgp_kernels::lw_solver_noscat_GaussQuad(
                &ncol, &nlay, &ngpt, &top_at_1, &n_quad_angs,
                const_cast<Real*>(gauss_Ds_subset.ptr()),
                const_cast<Real*>(gauss_wts_subset.ptr()),
                const_cast<Real*>(tau.ptr()),
                const_cast<Real*>(lay_source.ptr()),
                const_cast<Real*>(lev_source_inc.ptr()),
                const_cast<Real*>(lev_source_dec.ptr()),
                const_cast<Real*>(sfc_emis_gpt.ptr()),
                const_cast<Real*>(sfc_source.ptr()),
                gpt_flux_up.ptr(),
                gpt_flux_dn.ptr(),
                sfc_source_jac.ptr(),
                gpt_flux_up_jac.ptr());
        ...
        const int n_gauss_angles)
{
    ...
    Array<Real,2> sfc_emis_gpt({ncol, ngpt});
    ...
    Array<Real,2> gauss_Ds_subset = gauss_Ds.subset(
            {{ {1, n_quad_angs}, {n_quad_angs, n_quad_angs} }});
    Array<Real,2> gauss_wts_subset = gauss_wts.subset(
            {{ {1, n_quad_angs}, {n_quad_angs, n_quad_angs} }});

    // For now, just pass the arrays around.
    Array<Real,2> sfc_src_jac(sources.get_sfc_source().get_dims());
    Array<Real,3> gpt_flux_up_jac(gpt_flux_up.get_dims());
